...
function leagueController(){
    ...
    function getAllLeagues(request, response){
        ...
        .catch(function(error){
            log.logSeparator(console.error, 'FATAL --> FAT_030 - Fatal error on getting leagues from DB.');
            log.logSeparator(console.error, error);
            response.status(500).send(new responseMessage('FAT_030', 'FATAL --> Fatal error on getting leagues from DB. Check immediately console and logs.'));
        });
    }
